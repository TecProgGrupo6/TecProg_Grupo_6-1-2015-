...
public abstract class Player extends Actor{
		...
		for ( int x = -1 ; x<= 1 ; x++ ){
			...
			for ( int y = 1 ; y>= 0 ; y-- ){
				...
				if ( x>= 0&& x< Game.COLUMNS&& y>= 0&& y< Game.ROWS ){
					...
					Node adjacent = map.getNode ( getAxisX ()+ x , getAxisY ()+ y );
					if ( adjacent!= null ){
	...
	public void printEnemiesList(){
		...
		for ( Enemy enemy : GameplayState.getEnemyGroup ().getEnemies () ){
			...
			System.out.println ( enemy.toString ().substring ( enemy.toString ().lastIndexOf ( '.' ) )+ " at "
					+ enemy.getAxisX ()+ ","+ enemy.getAxisY ()+ " v2p="+ enemy.getVisibleToPlayer ()+ " ,aop="
	...
	public Node setDirection(){
		...
		if ( downLeftDirection&& getAxisY ()>= 1&& getAxisX ()>= 1 ){
		...
		if ( upLeftDirection&& getAxisY ()< Game.ROWS- 1&& getAxisX ()>= 1 ){
